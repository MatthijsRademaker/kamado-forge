...
Automatically archive a completed OpenSpec change after implementation, review, repair, and final validation are already complete.

This workflow state is the terminal worker outcome state. It must call `report_work_outcome` exactly once before stopping. Assistant prose or JSON is not terminal authority.
...
1. **Extract change name**
   ...
   Use the first token of `$ARGUMENTS` as the exact change name. Do NOT prompt, infer, or ask for selection. If `$ARGUMENTS` is empty, call `report_work_outcome` with `failed` and stop.

2. **Check if change directory exists**

   Verify `openspec/changes/<name>/` exists on disk.

   - If the directory does not exist: the change has likely already been archived. Treat this as successful idempotent completion — call `report_work_outcome` with `finished` and stop.
...
3. **Check if archive target already exists**
   ...
   - If the archive target already exists: treat as idempotent success. The change was already archived under today's date. Call `report_work_outcome` with `finished` and stop.
...
4. **Archive using openspec CLI**
   ...
   **If the command succeeds:** call `report_work_outcome` with `finished` and stop.
...
5. **Self-heal openspec failures**
   ...
   - Change directory missing (already archived) → call `report_work_outcome` with `finished`.
   - Archive target already exists → call `report_work_outcome` with `finished`.
   ...
   **5d. Retry archive**
   ...
   If it succeeds → call `report_work_outcome` with `finished` and stop.
   ...
   **5e. Second retry (if first self-heal failed)**
   ...
   If the second retry succeeds → call `report_work_outcome` with `finished` and stop.
   ...
   **5f. Manual archive (last-resort fallback)**
   ...
   4. Call `report_work_outcome` with `finished` and stop.

   If even manual archive fails (filesystem error, permission error):
   call `report_work_outcome` with `failed`, include the error verbatim in the tool message/details, and stop.

   **5g. Non-spec failures (filesystem, permissions)**

   If the error is NOT about spec sync (e.g. disk full, permission denied,
   git lock conflict), call `report_work_outcome` with `failed`, include the error verbatim in the tool message/details, and stop. Do NOT attempt self-heal for non-spec failures.

**Terminal outcome**

- On successful or idempotent completion, call `report_work_outcome` with `finished` exactly once.
- On unexpected failure, call `report_work_outcome` with `failed` exactly once and include the error.
- Do not emit assistant JSON as a substitute for the outcome tool. The swarm runtime accepts only the outcome artifact written by `report_work_outcome`.
