...
<requirements>
<requirement>Run `git diff <base_sha>...HEAD` using the merge-base three-dot comparison to understand the current branch changes. Never use a two-dot comparison such as `git diff origin/main..HEAD`: against a moving base it reports commits already merged into the base as changes on this branch, manufacturing false scope and false deletions.</requirement>
...
<requirement>Preserve runtime publication-failure comments (`source=runtime_publication_failure`) with their originating run ID, publication stage, classification, recovery attempt/result, session ID, and exact actionable output. Prioritize these failures before unrelated review work because they blocked publication of the current branch.</requirement>
...
<planning_inputs>
  ...
  <review_feedback>
    ...
    <item>
      ...
      <source_metadata>
        <author>...</author>
        <comment_source>runtime_publication_failure | review | agent_run | other | none</comment_source>
