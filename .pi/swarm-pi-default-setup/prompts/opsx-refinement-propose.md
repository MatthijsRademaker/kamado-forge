...
## Validate-before-submit loop

1. Draft complete synthesis JSON using opsx-propose discipline.
2. Call `validate_room_phase_result` with the draft.
3. If validation returns strict OpenSpec errors, repair the draft without changing accepted scope or traceability.
4. Repeat `validate_room_phase_result` until it returns `valid: true`.
5. Call `submit_room_phase_result` only with the exact draft that passed validation.

Never use submission as the first validation attempt. A strict-validation failure must be repaired and revalidated before submission.

## Structured-Output Checklist

Before calling `validate_room_phase_result`, verify:
