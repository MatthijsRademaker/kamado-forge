...
|---|---|---|
| 1 (highest) | `.opencode/commands/swarm-execute.md` | Scope discipline, pre-edit inspection, domain resolution, verification, subagent delegation |
| 2 | `.opencode/commands/opsx-archive-auto.md` | Archive automation, task-requirement completeness checks before saving |
| 3 | `.opencode/commands/opsx-apply.md` | Implementation precision, inspection depth, output structure |
...
|---|---|---|
| Plan missed required task domains | `opsx-archive-auto.md` | Strengthen review criteria: implementation must cover all task requirements listed in the task request |
| Plan was too verbose / wasted tokens | `opsx-apply.md` | Remove redundant sections, tighten output format |
| Worker made changes outside task scope | `swarm-execute.md` | Strengthen scope rules; require explicit "in scope / out of scope" check before editing |
| Worker skipped required files | `swarm-execute.md` | Require full file list derived from the plan + task request before editing begins |
| Worker introduced mechanical errors (indentation, duplicate calls) | `swarm-execute.md` | Strengthen verification requirements: typecheck/lint after every phase |
| Worker touched unrelated config files | `swarm-execute.md` | Add explicit "do not touch files outside the plan scope" rule |
| Worker changed model config or opencode settings | `swarm-execute.md` | Add explicit "do not modify opencode.jsonc, agent configs, or provider settings" rule |
| Review failed to catch a weak plan | `opsx-archive-auto.md` | Require explicit per-requirement check in review output |
| Plan had good content but reviewer still rejected with valid reasons | `swarm-execute.md` | The worker didn't follow the OpenSpec change faithfully |
...
**Alignment issues** (scope creep, missing requirements) → `swarm-execute.md` scope rules.
**Logic issues** (bugs, duplicates, broken code) → `swarm-execute.md` verification rules.
**Style issues** (formatting, indentation) → `swarm-execute.md` verification rules (add `go fmt` or equivalent after edits).
**Multiple issues across unrelated files** → `swarm-execute.md` scope discipline.
...
| `.opencode/commands/opsx-apply.md` | Apply phase prompt |
| `.opencode/commands/opsx-archive-auto.md` | Archive phase prompt |
| `.opencode/commands/swarm-execute.md` | Rework execution prompt |
