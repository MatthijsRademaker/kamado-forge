...
export function createApiDispatcher({ getHealth, liveCookRepository }: ApiDispatcherDependencies) {
  return (request: Request): Response | Promise<Response> => {
    ...
    if (url.pathname === createLiveDraftRoute.runtimePath)
      return dispatchDraftCreation(request, url, liveCookRepository);

    const activationMatch = /^\/api\/drafts\/([^/]+)\/activate$/.exec(url.pathname);
    if (activationMatch)
      return dispatchActivation(request, url, decodePathSegment(activationMatch[1] ?? ""), liveCookRepository);
    if (url.pathname === getActiveLiveSessionRoute.runtimePath)
      return dispatchActiveSession(request, url, liveCookRepository);

    const commandMatch = /^\/api\/live-session\/(advance|return|pause|resume|complete|cancel)$/.exec(url.pathname);
    if (commandMatch) return dispatchCommand(request, url, commandMatch[1] as LiveCookAction, liveCookRepository);
