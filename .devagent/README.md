...
| **Spec Writer** | Formulates task specifications during refinement |
| **Room Controller** | Runs the deterministic refinement room workflow |
...
`.devagent/.env` is local runtime configuration for this checkout. It stores project identity (`PROJECT_ID`), startup defaults (ports and feature flags), and optional image overrides (`MANAGER_IMAGE`, `TASK_REACTIVE_IMAGE`, `DASHBOARD_IMAGE`, `DOCS_IMAGE`, `TRAEFIK_IMAGE`). Re-run `swarm init` in a new clone to recreate it; it does not need to be committed.

## Auth Setup

All authentication and model configuration happens in the dashboard onboarding gate:

1. Run `swarm up` and open `http://localhost:18080/dashboard/`.
2. The **Auth Setup** phase connects Docker, hosting provider, and Pi credentials.
3. The **Model Selection** phase configures which models the agents use (written to `.pi/settings.json`).

Pi workers receive credentials through Docker volumes managed by the runtime — do not export provider API keys in your shell; the dashboard gate is the single configuration path.
...
`swarm up` launches the manager, dashboard, docs, and supporting runtime services.

Manual image overrides remain supported in `.devagent/.env`. Until public image endpoints are available everywhere, override them in `.devagent/.env` using `MANAGER_IMAGE`, `TASK_REACTIVE_IMAGE`, `DASHBOARD_IMAGE`, `DOCS_IMAGE`, and `TRAEFIK_IMAGE`.
...
```bash
# Infrastructure
swarm up                            # Start the runtime (manager, dashboard, docs)
swarm down                          # Stop the runtime (manager and all agents)
...
- **Auth Setup / Onboarding** — Connect Docker, hosting provider, and Pi, select models, and run the guided onboarding steps
