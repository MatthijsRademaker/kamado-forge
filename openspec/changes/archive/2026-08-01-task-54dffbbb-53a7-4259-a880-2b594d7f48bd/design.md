...
### Pinned initial contracts

The initial production adapter uses OpenAI's HTTPS Responses API through one non-streaming `fetch` call. `COACH_PROVIDER` must equal `openai`, `COACH_MODEL` must be a non-blank model identifier, and `OPENAI_API_KEY` must be non-blank. These variables are server-only and must never use a `VITE_` prefix. Model choice is configuration-only: the adapter sends the exact configured model and does not substitute a default. Missing or invalid configuration leaves health and non-coach routes available, but every coach request fails before context lookup with the safe configuration error below.

The public `POST /api/coach` request is `{ messages }`, where `messages` contains 1–20 strict objects, each role is `user` or `assistant`, each trimmed content value is 1–2,000 characters, total content is at most 12,000 characters, and the final role is `user`. The success envelope is `{ data: { message, suggestions } }`; `message` is 1–4,000 non-blank characters and `suggestions` contains at most four strict advisory items. An item is either `{ kind: "next_action", title, rationale }` or `{ kind: "caution", title, rationale }`, with a 1–120 character title and 1–500 character rationale. No model, provider, request ID, token usage, upstream body, URL, or vendor metadata is public.

`ContextSnapshotV1` is `{ version: 1, activeSession }`. An active session contains only `id`, `status`, `title`, `currentStep`, `targets`, and `recentNotes`. `currentStep` contains `id`, `ordinal`, `title`, `instructions`, and `durationMinutes`. `targets` contains the complete planned dome range and nullable planned food temperature. `recentNotes` spans every authoritative execution visit, contains at most five items, and is newest-first by `createdAt`, then execution ordinal, then note ordinal; each item contains `id`, `stepId`, `stepTitle`, `content`, and `createdAt`. Timing, progress, setup fields, execution elapsed time, and next-step fields are excluded. No active session is exactly `{ version: 1, activeSession: null }`.

The provider receives exactly two advisory function schemas: `recommend_next_action` and `highlight_cook_risk`. Both accept strict `{ title, rationale }` arguments with the same bounds as public suggestions. Their typed outputs map to `next_action` and `caution` suggestions respectively. Tool calls are interpreted only as returned advisory data; there is no executor, session ID argument, or operation that can advance a step, change a target, add a note, alter timing, or change status.

Public failures use the shared envelope with an empty `issues` array: invalid or unsupported configuration is HTTP 503 `COACH_CONFIGURATION_ERROR` / `Coach provider is not configured`; provider rejection is HTTP 502 `COACH_PROVIDER_REJECTED` / `Coach provider rejected the request`; network or timeout failure is HTTP 503 `COACH_PROVIDER_UNAVAILABLE` / `Coach provider is unavailable`; and malformed provider output is HTTP 502 `COACH_PROVIDER_INVALID_RESPONSE` / `Coach provider returned an invalid response`. Request validation remains HTTP 400 `VALIDATION_ERROR` / `Request validation failed` with deterministically sorted issues.
