...
The implementation SHALL limit behavior changes to the frontend foundation. It MUST add required local font dependencies in `frontend/package.json` with matching root `bun.lock` entries, leave `App.vue`, feature pages, navigation, app-specific components, backend behavior, and documentation unchanged, and pass `scripts/precommit-run` including the frozen-lockfile frontend build.

#### Scenario: Repository verification succeeds

- **WHEN** the implementation is complete
- **THEN** `scripts/precommit-run` completes with the font manifest and root lockfile in sync
